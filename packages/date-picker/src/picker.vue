...
import { formatDate, parseDate, isDateObject, getWeekNumber } from 'tema-ui/src/utils/date-util';
    ...
    handleKeydown(event) {
      ...
      if (keyCode === 9) {
        if (!this.ranged) {
          ...
          setTimeout(() => {
            if (this.refInput.indexOf(document.activeTmDesign) === -1) {
              this.pickerVisible = false;
              this.blur();
              event.stopPropagation();
            }
