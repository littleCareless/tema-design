import Vue from 'vue';
import { hasClass, addClass, removeClass } from 'tema-ui/src/utils/dom';
...
export default {
                    ...
                    <div class={ ['cell', column.filteredValue && column.filteredValue.length > 0 ? 'highlight' : '', column.labelClassName] }>
                      ...
                      }
                      {
                        column.filterable ? (<span
                          ...
                          <i class={ ['tm-icon-arrow-down', column.filterOpened ? 'tm-icon-arrow-up' : ''] }></i>
                        </span>) : ''
                      }
                    </div>
                  </th>))
  ...
  mounted() {
    // nextTick 是有必要的 https://github.com/TmemeFE/element/pull/11311
    this.$nextTick(() => {
  ...
  methods: {
    ...
    handleMouseDown(event, column) {
      ...
      if (this.draggingColumn && this.border) {
        ...
        const table = this.$parent;
        const tableTm = table.$el;
        const tableLeft = tableTm.getBoundingClientRect().left;
        const columnTm = this.$el.querySelector(`th.${column.id}`);
        const columnRect = columnTm.getBoundingClientRect();
        const minLeft = columnRect.left - tableLeft + 30;

        addClass(columnTm, 'noclick');
