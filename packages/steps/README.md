...
```javascript
import Vue from 'vue'
import TmStep from 'element-steps'
...
```javascript
import Vue from 'vue'
import { TmSteps, TmStep } from 'element-steps'

Vue.component('el-steps', TmSteps)
Vue.component('el-step', TmStep)
```
...
| icon | 图标 | TmDesign Icon 提供的图标，如果要使用自定义图标可以通过 slot 方式写入 | string | — |
