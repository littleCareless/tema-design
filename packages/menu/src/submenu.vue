<script>
  import TmCollapseTransition from 'tema-ui/src/transitions/collapse-transition';
  ...
  export default {
    ...
    computed: {
      ...
      isFirstLevel() {
        ...
        while (parent && parent !== this.rootMenu) {
          if (['TmSubmenu', 'TmMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
            isFirstLevel = false;
            break;
          } else {
    ...
    methods: {
      ...
      handleMouseenter(event, showTimeout = this.showTimeout) {
        ...
        this.dispatch('TmSubmenu', 'mouse-enter-child');
      ...
      handleMouseleave(deepDispatch = false) {
        ...
        this.dispatch('TmSubmenu', 'mouse-leave-child');
        clearTimeout(this.timeout);
        this.timeout = setTimeout(() => {
        ...
        if (this.appendToBody && deepDispatch) {
          if (this.$parent.$options.name === 'TmSubmenu') {
    ...
    render(h) {
      ...
      ) ? 'tm-icon-arrow-down' : 'tm-icon-arrow-right';
