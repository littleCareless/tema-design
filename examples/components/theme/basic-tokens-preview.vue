...
<template>
  <div class="component-preview">
    ...
    <div class="color">
      <tm-row :gutter="12">
        <tm-col :span="4" v-for="(color, key) in colorLine" :key="key">
...
export default {
  created() {
    ...
    bus.$on(ACTION_COMPONECT_SELECT, (val) => {
      this.$nextTick(() => {
        const getSelectTmDesign = Array.from(document.querySelectorAll('h4')).filter((el) => (el.innerText.toLowerCase() === val));
        if (getSelectTmDesign[0]) {
          const elementTop = getSelectTmDesign[0].getBoundingClientRect().top;
