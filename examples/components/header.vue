...
<script>
  ...
  import { getTestTme } from './theme/loader/api.js';
  import bus from '../bus';
  ...
  export default {
    ...
    mounted() {
      getTestTme()
        .then(() => {
          this.$isTme = true;
          ga('send', 'event', 'DocView', 'Tme', 'Inner');
        })
        .catch((err) => {
          ga('send', 'event', 'DocView', 'Tme', 'Outer');
          console.error(err);
        });
