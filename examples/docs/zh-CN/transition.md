...
TmDesign 内应用在部分组件的过渡动画，你也可以直接使用。在使用之前请阅读 [transition 组件文档](https://cn.vuejs.org/v2/api/#transition) 。

### fade 淡入淡出

:::demo 提供 `tm-fade-in-linear` 和 `tm-fade-in` 两种效果。
```html
<template>
  <div>
...
```js
// fade/zoom 等
import 'tema-ui/lib/theme-chalk/base.css';
// collapse 展开折叠
import CollapseTransition from 'tema-ui/lib/transitions/collapse-transition';
import Vue from 'vue'
