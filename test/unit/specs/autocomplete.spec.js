...
describe('Autocomplete', () => {
  ...
  it('create', done => {
    ...
    let elm = vm.$el;
    let inputTmm = elm.querySelector('input');
    inputTmm.focus();

    expect(inputTmm.getAttribute('placeholder')).to.be.equal('请输入内容autocomplete1');
  ...
  it('select', done => {
    ...
    const elm = vm.$el;
    const inputTmm = elm.querySelector('input');
    const spy = sinon.spy();
    ...
    setTimeout(_ => {
      ...
      setTimeout(_ => {
        expect(inputTmm.value).to.be.equal('Hot honey 首尔炸鸡（仙霞路）');
  ...
  it('props', done => {
    ...
    setTimeout(_ => {
      ...
      suggestionList[1].click();
      setTimeout(_ => {
        expect(inputTmm.value).to.be.equal('上海市长宁区淞虹路661号');
  ...
  it('highlight', done => {
    ...
    const autocomplete = vm.$refs.autocomplete;
    const inputTmm = autocomplete.$el.querySelector('input');
