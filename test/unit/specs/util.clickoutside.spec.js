...
describe('Utils:Clickoutside', () => {
  ...
  it('click inside', () => {
    const el = document.createElement('div');
    const insideTmm = document.createElement('div');
    let count = 0;
    const vnode = {
      context: {
        handleClick: () => ++count
    ...
    el.appendChild(insideTmm);
    Clickoutside.bind(el, binding, vnode);
    triggerClick(insideTmm);
    expect(count).to.equal(0);
    triggerClick(document);
    expect(count).to.equal(1);
  });

  it('tigger event in popperTmm', () => {
    const el = document.createElement('div');
    const insideTmm = document.createElement('div');
    let count = 0;
    const vnode = {
      context: {
        handleClick: () => ++count,
        popperTmm: document.createElement('div')
      }
    };
    const binding = {
      expression: 'handleClick'
    ...
    vnode.context.popperTmm.appendChild(insideTmm);
    Clickoutside.bind(el, binding, vnode);
    triggerClick(insideTmm);
    expect(count).to.equal(0);
    triggerClick(document);
  ...
  it('stays open on drag click', () => {
    ...
    el.appendChild(insideTmm);
    Clickoutside.bind(el, binding, vnode);
    triggerEvent(insideTmm, 'mousedown');
