...
describe('Calendar', () => {
  ...
  it('create', async() => {
    ...
    const titleTm = vm.$el.querySelector('.el-calendar__title');
    expect(/2019.*4/.test(titleTm.innerText)).to.be.true;
    expect(vm.$el.querySelectorAll('thead th').length).to.equal(7);
    const rows = vm.$el.querySelectorAll('.el-calendar-table__row');
    expect(rows.length).to.equal(6);
    rows[5].firstTmDesignChild.click();

    await waitImmediate();

    expect(/2019.*5/.test(titleTm.innerText)).to.be.true;
  ...
  it('range', () => {
    ...
    const titleTm = vm.$el.querySelector('.el-calendar__title');
    expect(/2019.*3/.test(titleTm.innerText)).to.be.true;
    const rows = vm.$el.querySelectorAll('.el-calendar-table__row');
  ...
  it('range tow monthes', async() => {
    ...
    const cell = rows[rows.length - 1].firstTmDesignChild;
    cell.click();
  ...
  it('firstDayOfWeek', async() => {
    ...
    expect(head.firstTmDesignChild.innerText).to.be.equal('日');
    expect(head.lastTmDesignChild.innerText).to.be.equal('六');
    const firstRow = vm.$el.querySelector('.el-calendar-table__row');
    expect(firstRow.firstTmDesignChild.innerText).to.be.equal('31');
    expect(firstRow.lastTmDesignChild.innerText).to.be.equal('6');
  });
  ...
  it('firstDayOfWeek in range mode', async() => {
    ...
    expect(head.lastTmDesignChild.innerText).to.be.equal('六');
    const firstRow = vm.$el.querySelector('.el-calendar-table__row');
    expect(firstRow.firstTmDesignChild.innerText).to.be.equal('3');
    expect(firstRow.lastTmDesignChild.innerText).to.be.equal('9');
  });
});
