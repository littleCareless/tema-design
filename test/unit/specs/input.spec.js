...
describe('Input', () => {
  ...
  it('create', async() => {
    ...
    let inputTmm = vm.$el.querySelector('input');
    inputTmm.focus();
    expect(vm.inputFocus).to.be.true;
    expect(inputTmm.getAttribute('placeholder')).to.equal('请输入内容');
    expect(inputTmm.value).to.equal('input');
    expect(inputTmm.getAttribute('minlength')).to.equal('3');
    expect(inputTmm.getAttribute('maxlength')).to.equal('5');

    vm.input = 'text';
    await waitImmediate();
    expect(inputTmm.value).to.equal('text');
  });
  ...
  it('default to empty', () => {
    ...
    let inputTmm = vm.$el.querySelector('input');
    expect(inputTmm.value).to.equal('');
  });
  ...
  it('Input contains Select and append slot', async() => {
    ...
    await wait();
    const suffixTm = document.querySelector('.input-with-select > .el-input__suffix');
    expect(suffixTm).to.not.be.null;
    expect(suffixTm.style.transform).to.not.be.empty;
  });
  ...
  describe('Input Events', () => {
    ...
    it('event:change', async() => {
      ...
      const inputTmm = vm.$el.querySelector('input');
      const simulateEvent = (text, event) => {
        inputTmm.value = text;
        inputTmm.dispatchEvent(new Event(event));
      };
  ...
  it('limit input and show word count', async() => {
    ...
    const inputTmm1 = vm.$el.querySelector('.test-text');
    const inputTmm2 = vm.$el.querySelector('.test-textarea');
    const inputTmm3 = vm.$el.querySelector('.test-password');
    const inputTmm4 = vm.$el.querySelector('.test-initial-exceed');

    expect(inputTmm1.querySelectorAll('.el-input__count').length).to.equal(0);
    expect(inputTmm2.querySelectorAll('.el-input__count').length).to.equal(1);
    expect(inputTmm3.querySelectorAll('.el-input__count').length).to.equal(0);
    expect(inputTmm4.classList.contains('is-exceed')).to.true;

    vm.show = true;
    await waitImmediate();
    expect(inputTmm1.querySelectorAll('.el-input__count').length).to.equal(1);

    vm.input4 = '1';
    await waitImmediate();
    expect(inputTmm4.classList.contains('is-exceed')).to.false;
  });
});
