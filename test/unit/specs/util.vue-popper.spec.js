import VuePopper from 'tema-ui/src/utils/vue-popper';
...
const Popper = Object.assign({}, VuePopper, {
  ...
  created() {
    this.popperTmm = document.createElement('div');
    this.referenceTmm = document.createElement('div');
  }
});
...
describe('Utils:VuePopper', () => {
  it('set popper not reference', () => {
    ...
    vm.createPopper();
    expect(vm.popperTmm).to.exist;
    expect(vm.referenceTmm).to.not.exist;
  ...
  it('set reference not popper', () => {
    ...
    vm.createPopper();
    expect(vm.referenceTmm).to.exist;
    expect(vm.popperTmm).to.not.exist;
    expect(vm.popperJS).to.not.exist;
  });
  ...
  it('appendToBody set false', () => {
    ...
    vm.createPopper();
    expect(document.body.contains(vm.popperTmm)).to.false;
  });
  ...
  it('destroy', () => {
    ...
    vm.createPopper();
    expect(document.body.contains(vm.popperTmm)).to.true;
    vm.$destroy();
    expect(document.body.contains(vm.popperTmm)).to.false;
  });
});
