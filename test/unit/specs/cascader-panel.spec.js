...
describe('CascaderPanel', () => {
  ...
  it('with default value', async() => {
    ...
    expect(getOptions(el, 2)[0].querySelector('.tm-icon-check')).to.exist;
  });
  ...
  it('dynamic loading', async() => {
    ...
    firstOption.click();
    await waitImmediate();
    expect(firstOption.querySelector('i').className).to.includes('tm-icon-loading');
    await wait(1000);
    expect(firstOption.querySelector('i').className).to.includes('tm-icon-arrow-right');
    expect(getMenus(el).length).to.equal(2);
    getOptions(el, 1)[0].click();
    await wait(1000);
    getOptions(el, 2)[0].click();
