...
describe('MessageBox', () => {
  ...
  it('custom icon', done => {
    MessageBox({
      type: 'warning',
      iconClass: 'tm-icon-question',
      message: '这是一段内容'
    });
    setTimeout(() => {
      const icon = document.querySelector('.el-message-box__status');
      expect(icon.classList.contains('tm-icon-question')).to.true;
      done();
    }, 300);
  });
  ...
  it('prompt', done => {
    ...
    setTimeout(() => {
      ...
      const haveFocus = messageBox.$el.querySelector('input').isSameNode(document.activeTmDesign);
  ...
  it('prompt: focus on textarea', done => {
    ...
    setTimeout(() => {
      const messageBox = document.querySelector('.el-message-box__wrapper').__vue__.$parent;
      const haveFocus = messageBox.$el.querySelector('textarea').isSameNode(document.activeTmDesign);
      expect(haveFocus).to.true;
      done();
    }, 700);
  });
  ...
  it('beforeClose', done => {
    ...
    setTimeout(() => {
      document.querySelector('.el-message-box__wrapper .tm-button--primary').click();
      setTimeout(() => {
  ...
  describe('promise', () => {
    ...
    it('reject', done => {
      ...
      setTimeout(() => {
        document.querySelector('.el-message-box__wrapper .tm-button').click();
      }, 50);
    });
  });
});
