...
describe('Dialog', () => {
  ...
  it('render correct content', done => {
    ...
    setTimeout(() => {
      const footerBtns = vm.$el.querySelectorAll('.el-dialog__footer .tm-button');
  ...
  it('open and close', done => {
    ...
    const dialogTm = vm.$children[0].$el;
    expect(getComputedStyle(dialogTm).display).to.equal('none');
    vm.visible = true;
    setTimeout(() => {
      expect(getComputedStyle(dialogTm).display).to.not.equal('none');
      vm.visible = false;
      setTimeout(() => {
        expect(getComputedStyle(dialogTm).display).to.equal('none');
        done();
  ...
  describe('props', () => {
    ...
    it('fullscreen', () => {
      ...
      const dialogTm = vm.$el.querySelector('.el-dialog');
      expect(dialogTm.classList.contains('is-fullscreen')).to.true;
      expect(dialogTm.style.width).to.be.empty;
    });
