...
describe('Steps', () => {
  ...
  it('space', async() => {
    ...
    await waitImmediate();
    const stepTmm = vm.$el.querySelector('.el-step');
    const stepTmm2 = vm2.$el.querySelector('.el-step');
    expect(getComputedStyle(stepTmm).flexBasis).to.equal('50%');
    expect(getComputedStyle(stepTmm2).flexBasis).to.equal('100px');
  });
  ...
  it('vertical:height', async() => {
    ...
    await waitImmediate();
    const stepTmm = vm.$el.querySelector('.el-step');
    expect(getComputedStyle(stepTmm).flexBasis).to.equal('200px');
  });
